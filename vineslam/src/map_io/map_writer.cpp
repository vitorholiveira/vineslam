...
MapWriter::MapWriter(const Parameters& params, const std::string& str)
{
  file_path_ = str;
}
...
void MapWriter::writeToFile(OccupancyMap* grid_map, const Parameters& params)
{
      ...
      for (float j = ymin; j < ymax - grid_map->resolution_;)
      {
        ...
        else
        {
          for (const auto& landmark : *l_landmarks)
          {
            ...
            xmlfile << TAB << TAB << TAB << TAB << open(LABEL) << landmark.second.label_ << close(LABEL) << ENDL;
  ...
  for (const auto& plane : grid_map->planes_)
  {
    ...
    {
      xmlfile << TAB << TAB << TAB << TAB << open(VINESLAM_POINT) << ENDL;
      ...
      xmlfile << TAB << TAB << TAB << TAB << close(VINESLAM_POINT) << ENDL;
    }
