...
MapLayer::MapLayer(const Parameters& params, const Pose& origin_offset)
{
  ...
  int map_size =
      static_cast<int>(std::round(((width_ + resolution_) / resolution_) * ((lenght_ + resolution_) / resolution_)));
...
bool MapLayer::update(const Corner& old_corner, const Corner& new_corner)
{
  ...
  if (l_corners != nullptr)
  {
    ...
    for (size_t i = 0; i < l_corners->size(); i++)
    {
      ...
      if (std::fabs(l_corner.pos_.x_ - old_corner.pos_.x_) < 1e-5 &&
          std::fabs(l_corner.pos_.y_ - old_corner.pos_.y_) < 1e-5 &&
          std::fabs(l_corner.pos_.z_ - old_corner.pos_.z_) < 1e-5)
      {
...
#if VERBOSE == 1
  std::cout << "WARNING (MapLayer::update): Trying to update a corner that is "
               "not on the map... "
            << std::endl;
#endif
  return false;
...
bool MapLayer::update(const Planar& old_planar, const Planar& new_planar)
{
  ...
  if (l_planars != nullptr)
  {
    ...
    for (size_t i = 0; i < l_planars->size(); i++)
    {
      ...
      if (std::fabs(l_planar.pos_.x_ - old_planar.pos_.x_) < 1e-5 &&
          std::fabs(l_planar.pos_.y_ - old_planar.pos_.y_) < 1e-5 &&
          std::fabs(l_planar.pos_.z_ - old_planar.pos_.z_) < 1e-5)
      {
...
#if VERBOSE == 1
  std::cout << "WARNING (MapLayer::update): Trying to update a planar that is "
               "not on the map... "
            << std::endl;
#endif
  return false;
...
void MapLayer::deallocateAllMem()
{
  for (size_t idx = 0; idx < cell_vec_.size(); idx++)
  {
    if (cell_vec_[idx].data != nullptr)
    {
      if (cell_vec_[idx].data->candidate_corner_features_ != nullptr)
      {
        delete cell_vec_[idx].data->candidate_corner_features_;
        cell_vec_[idx].data->candidate_corner_features_ = nullptr;
      }
      if (cell_vec_[idx].data->candidate_planar_features_ != nullptr)
      {
        delete cell_vec_[idx].data->candidate_planar_features_;
        cell_vec_[idx].data->candidate_planar_features_ = nullptr;
      }
      if (cell_vec_[idx].data->corner_features_ != nullptr)
      {
        delete cell_vec_[idx].data->corner_features_;
        cell_vec_[idx].data->corner_features_ = nullptr;
      }
      if (cell_vec_[idx].data->planar_features_ != nullptr)
      {
        delete cell_vec_[idx].data->planar_features_;
        cell_vec_[idx].data->planar_features_ = nullptr;
      }
      if (cell_vec_[idx].data->landmarks_ != nullptr)
      {
        delete cell_vec_[idx].data->landmarks_;
        cell_vec_[idx].data->landmarks_ = nullptr;
      }
      if (cell_vec_[idx].data->surf_features_ != nullptr)
      {
        delete cell_vec_[idx].data->surf_features_;
        cell_vec_[idx].data->surf_features_ = nullptr;
      }
      if (cell_vec_[idx].data->is_occupied_ != nullptr)
      {
        delete cell_vec_[idx].data->is_occupied_;
        cell_vec_[idx].data->is_occupied_ = nullptr;
      }
      delete cell_vec_[idx].data;
      cell_vec_[idx].data = nullptr;
    }
  }
}
  ...
  else
  {
    ...
    if (l_image_features != nullptr)
    {
      ...
      for (size_t i = 0; i < l_image_features->size(); i++)
      {
        ...
        if (std::fabs(l_image_feature.pos_.x_ - old_image_feature.pos_.x_) < 1e-5 &&
            std::fabs(l_image_feature.pos_.y_ - old_image_feature.pos_.y_) < 1e-5 &&
            std::fabs(l_image_feature.pos_.z_ - old_image_feature.pos_.z_) < 1e-5)
        {
...
void OccupancyMap::deallocateAllMem()
{
  for (auto& layer : layers_map_)
  {
    layer.second.deallocateAllMem();
  }
}
