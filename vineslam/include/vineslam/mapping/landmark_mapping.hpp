...
public:
  ...
  void init(const Pose& pose, const std::vector<float>& bearings, const std::vector<float>& pitches,
            const std::vector<float>& depths, const std::vector<int>& labels, TopologicalMap& topological_map);
  ...
  void localMap(const Pose& cam_origin_pose, const std::vector<int>& labels, const std::vector<float>& bearings,
                ...
                TopologicalMap& topological_map, Pose robot_pose) const;
  void localMap(const Pose& cam_origin_pose, const std::vector<int>& labels, const std::vector<float>& bearings,
                const std::vector<float>& pitches, std::vector<SemanticFeature>& landmarks,
...
private:
  ...
  static std::pair<int, SemanticFeature> findCorr(const Point& l_pos, const int& label,
                                                  TopologicalMap& topological_map);
};
