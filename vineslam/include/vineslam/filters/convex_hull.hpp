...
struct QuickConvexHull
{
  typedef boost::geometry::model::point<double, 3, boost::geometry::cs::cartesian> boost_point;
  typedef boost::geometry::model::multi_point<boost_point> boost_polygon;

  static bool quickConvexHull(const Plane& plane, SemiPlane& semi_plane)
  {
    // ------------------------------------------------------------------
    // ----- Get local plane reference frame transformation matrix
    // ------------------------------------------------------------------
    ...
    {
      poly.emplace_back(pt.x_, pt.y_, pt.z_);
    }
    ...
    {
      double xx = it.get<0>(), yy = it.get<1>(), zz = it.get<2>();
      Point l_pt(xx, yy, zz);
      semi_plane.extremas_.push_back(l_pt);
    }
  ...
  static bool process(const Plane& plane, SemiPlane& semi_plane)
  {
    ...
    quickConvexHull(plane, semi_plane);

    // Convert stack to vector and save the output semi-plane
    Tf plane_ref = plane.local_ref_;
    std::vector<Point> extremas;
    for (auto& pt : semi_plane.extremas_)
    {
      pt = pt * plane_ref;
      if (std::isfinite(pt.x_) && std::isfinite(pt.y_) && std::isfinite(pt.z_))
      {
